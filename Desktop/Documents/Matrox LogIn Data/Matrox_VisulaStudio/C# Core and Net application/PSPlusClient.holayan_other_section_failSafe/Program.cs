        ...
        private static async Task ExecuteRequestAsync(UserRequest request)
        {
            ...
            if (additionSucceeded)
            {
                ...
                if ((unit.Urn == UnitURN.SV2Dec) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
                    ...
                    // Adding the other fail Safe

                    var piplineSettings = settings.Pipeline.Modules.First(m => m is SV2.Types.Decoder.PipelineSettings)
                       as SV2.Types.Decoder.PipelineSettings;

                    Console.WriteLine("the failSafe data is: " + piplineSettings.Settings.Failsafe);

                    var fileReader = new SV2.Types.Decoder.FileReader<SV2.Types.Decoder.FileSettingsLocalStorage>();

                    fileReader.Settings.FilePath = "holayan test"; ///"/failsafe/medias/Puppies.mp4";
                    fileReader.Settings.FriendlyName = "Puppies";
                    fileReader.Settings.NoLoop = false;

                    piplineSettings.Settings.Failsafe = fileReader;
