    ...
    internal class Program
    {
        ...
        private static async Task ExecuteRequestAsync(UserRequest request)
        {
            ...
            if (additionSucceeded)
            {
                ...
                if ((unit.Urn == UnitURN.SV2) && (unit.Capabilities == UnitCapabilities.CanEncode))
                {
                    // This is a Maevex 6100 encoder
                    ...
                    var context = unit.Context as SV2.Types.Context;
                    ...
                    var settings = context.Settings.DeepClone() as SV2.Types.Settings;
                    ...
                    // Enable and Disable the LocalPreview
                    settings.LocalPreview.IsEnabled = true;

                    // Note dealing with Tittle and ask about it is location
                    // settings.LocalPreview.Title = "HUSSEIN";

                    // Including Audio
                    settings.LocalPreview.IncludeAudio = true;

                    // specified a specfic Action Button ID
                    settings.LocalPreview.ActionButtonID = SV2.Types.ActionButtonID.Button1;


                    // To make an action on a device(reboot or apply settings),
                    // use the correct device controller (SV2Controller in this case).
                    var controller = UnitAccess.Instance.SV2Controller;
                    ...
                    if (resultTuple.Item1.IsSuccess)
                    {
                        ...
                        var settingsJustApplied = ((SV2.Types.Context)resultTuple.Item2.Context).Settings as SV2.Types.Settings;
                ...
                else if ((unit.Urn == UnitURN.Maevex1) && (unit.Capabilities == UnitCapabilities.CanEncode))
                {
                    // This is a Maevex 5100-series encoder
                    // The device information is available to all, but you must provide the correct password
                    // to control the device (reboot or apply new settings). This is called the "Environment password".
                    UnitAccess.Instance.MaevexPassword = request.Password;

                    // Once you found the latest snapshot of your board in the Units,
                    // its context is found in the Context property of the snapshot.
                    var context = unit.Context as Maevex.Types.ContextData;

                    // Get the settings (from the latest context).
                    // To make sure it’s not affected by another device update being received, do a
                    // DeepClone() of the settings, then use this copy from now on in this apply sequence.
                    var settings = context.Settings.DeepClone() as Maevex.Types.EncoderSettingsData;

                    // Ouput the current friendly name of the device.
                    Console.WriteLine("The device's friendly name is " + settings.FriendlyName);
                    ...
                    // To make an action on a device(reboot or apply settings),
                    // use the correct device controller(MaevexController in this case).
                    var controller = UnitAccess.Instance.MaevexController;
                    ...
                    // snapshot of the unit corresponding to the device's state right after the
                    // SetSettings (Item2)
                    if (resultTuple.Item1.IsCompleteSuccess())
                    {
                        Console.WriteLine("Successfully applied settings change.");

                        // Get the settings from the latest context again to see the setting change.
                        var settingsJustApplied = ((Maevex.Types.ContextData)resultTuple.Item2.Context).Settings as Maevex.Types.EncoderSettingsData;

                        Console.WriteLine("The device's friendly name is now " + settingsJustApplied.FriendlyName);
                    }
                    else
                    {
                        Console.WriteLine("The API returned " + resultTuple.Item1 + " attempting to apply a setting.");
                    }
                }
                else if ((unit.Urn == UnitURN.Maevex1) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
                    // This is a Maevex 5100-series decoder
                    // The device information is available to all, but you must provide the correct password
                    // to control the device (reboot or apply new settings). This is called the "Environment password".
                    UnitAccess.Instance.MaevexPassword = request.Password;
                    ...
                    // Get the settings (from the latest context).
                    // To make sure itâ€™s not affected by another device update being received, do a
                    // DeepClone() of the settings, then use this copy from now on in this apply sequence.
                    var settings = context.Settings.DeepClone() as Maevex.Types.DecoderSettingsData;
                    ...
                    if (resultTuple.Item1.IsCompleteSuccess())
                    {
                        ...
                        // Get the settings from the latest context again to see the setting change.
                        var settingsJustApplied = ((Maevex.Types.ContextData)resultTuple.Item2.Context).Settings as Maevex.Types.DecoderSettingsData;
                ...
                else if ((unit.Urn == UnitURN.SV2Dec) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
