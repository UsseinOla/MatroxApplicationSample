        ...
        private static async Task ExecuteRequestAsync(UserRequest request)
        {
            ...
            if (additionSucceeded)
            {
                ...
                if ((unit.Urn == UnitURN.SV2Dec) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
                    ...
                    // use the correctr device controller (SV2DecController in this case).
                    ...
                    // Downloading Logs 
                    string localPathForLogs = @"C:\Users\holayan\Desktop";

                    var logFileType = SV2.Types.LogFileType.Log;

                    var downloadLogs = await controller.DownloadLogFilesAsync(
                        unit.Uuid,
                        unit.UnitUri,
                        localPathForLogs,
                        logFileType,
                    ...
                    // Downloading Audits
                    string localPathForAudits = @"C:\Users\holayan\Desktop";

                    var logFileAudits = SV2.Types.LogFileType.Audit;

                    var downloadAudits = await controller.DownloadLogFilesAsync(
                        unit.Uuid,
                        unit.UnitUri,
                        localPathForAudits,
                        logFileAudits,
                    ...
                    // Deleteing device logs
                    var logFileTypexx = SV2.Types.LogFileType.Log;

                    var time = TimeSpan.FromSeconds(20);

                    var deleteLogFiles = await controller.DeleteLogFilesAsync(
                        unit.Uuid,
                        unit.UnitUri,
                        logFileTypexx,
                        CancellationToken.None,
                    ...
                    var debug = await controller.ConfigureDebugOptionsAsync(
                        unit.Uuid,
                        unit.UnitUri,
                        "PixelFormat"
                        );
