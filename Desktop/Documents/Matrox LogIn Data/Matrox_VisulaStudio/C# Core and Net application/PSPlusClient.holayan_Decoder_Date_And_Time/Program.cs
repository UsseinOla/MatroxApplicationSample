        ...
        private static async Task ExecuteRequestAsync(UserRequest request)
        {
            ...
            if (additionSucceeded)
            {
                 ...
                 if ((unit.Urn == UnitURN.SV2Dec) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
                    ...
                    var time = new SV2.Types.Decoder.ContextDetails.TimeSettings();

                    time.TimeZone = SV2.Types.DeviceTimeZone.Europe_Madrid;
                    time.NTPServerAddress = "holayanTest "; /*by defualt is: time.matrox.com */

                    /// assing all data to settings
