        ...
        private static async Task ExecuteRequestAsync(UserRequest request)
        {
            ...
            if (additionSucceeded)
            {
                ...
                if ((unit.Urn == UnitURN.SV2Dec) && (unit.Capabilities == UnitCapabilities.CanDecode))
                {
                    ...
                    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
                    /*
                     public Task<Result> UploadLocalFilesAsync(string uuid, UnitUri uri, List<FileUploadInfo> filePaths,
                    ulong maxPartSize, IProgress<DownloadFileProgressInfo> progress, CancellationToken cancelToken = default);

                     */
                    var fileData = new SV2DecController.FileUploadInfo("C:/Users/holayan/Desktop/Matrox/Matrox_videos/NewYork.mp4", "NewYork");
                    List<SV2DecController.FileUploadInfo> listFileData = new List<SV2DecController.FileUploadInfo>();
                    listFileData.Add(fileData);

                    var uploadfile = await controller.UploadLocalFilesAsync(
                        unit.Uuid,
                        unit.UnitUri,
                       listFileData,
                       1000000 /*locate Maximum file partSize to 1000000 bytes*/,
                       null,
